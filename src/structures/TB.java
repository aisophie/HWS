...
public class TB extends HWS{
	...
	private void getHead(HashMap<String,HashMap<String,Integer>> waMap,String cxt,String[] tokens,int row,int start,int end){
		...
		for(int i=start;i<end;i++){
			...
			double dc = (ccw - cc * cw / V) / Math.sqrt(ccw);
			if(dc>max){
				max = dc;
				idx = i;
			}
	...
	public HashMap toCellMap(String[] tokens){
		super.initCellMap();
		if(this.V==0.0){
			for(Object childMaps:this.extraMap.values()){
				this.V += (Integer)((HashMap)childMaps).get("|C|");
			}
		}
